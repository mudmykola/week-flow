...
export default defineEventHandler(async (event) => {
  ...
  const [managedTeam] = await db.select({ id: teams.id }).from(teams).where(and(eq(teams.id, goal.teamId), eq(teams.managerId, user.id)))
  if (goal.assigneeId !== user.id && !managedTeam) throw createError({ statusCode: 403 })
  const progress = body.status === 'done' ? 100 : body.progress
  await db.update(goals).set({ progress, status: body.status ?? (progress === 100 ? 'done' : 'active'), updatedAt: Date.now() }).where(eq(goals.id, id))
