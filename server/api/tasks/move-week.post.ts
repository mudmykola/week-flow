...
export default defineEventHandler(async (event) => {
  ...
  const [moving, target] = await Promise.all([
    db
      .select()
      .from(tasks)
      .where(and(eq(tasks.week, body.fromWeek), ne(tasks.status, 'done'), eq(tasks.ownerId, user.id))),
    db
      .select()
      .from(tasks)
      .where(and(eq(tasks.week, body.toWeek), eq(tasks.ownerId, user.id)))
  ])
  ...
  if (moving.length) {
    await Promise.all(
      moving.map((task) =>
        db.update(tasks).set({ week: body.toWeek, sort: nextSort[task.status]++ }).where(eq(tasks.id, task.id))
      )
    )
  }
