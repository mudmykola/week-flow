...
export default defineEventHandler(async (event) => {
  ...
  if (body.projectId) {
    ...
    if (!project) {
      const [member] = await db.select().from(projectMembers).where(and(eq(projectMembers.projectId, body.projectId), eq(projectMembers.userId, user.id)))
  ...
  await logActivity(event, { ownerId: user.id, actorId: user.id, action: 'task.created', entityType: 'task', entityId: task.id })
  return task
})
