...
export default defineEventHandler(async (event) => {
  ...
  const [invitation] = await db
    .select()
    .from(invitations)
    .where(and(eq(invitations.token, token), eq(invitations.status, 'pending'), gt(invitations.expiresAt, Date.now())))
  if (!invitation || invitation.email !== user.email.toLowerCase())
    throw createError({ statusCode: 404, statusMessage: 'Invitation not found' })
  await db.batch([
    db
      .insert(projectMembers)
      .values({ projectId: invitation.projectId, userId: user.id, role: invitation.role, createdAt: Date.now() })
      .onConflictDoUpdate({
