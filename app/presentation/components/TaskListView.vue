...
<template>
  <div class="app-container max-w-5xl">
    ...
    <div v-if="loading" class="space-y-2"><USkeleton v-for="i in 4" :key="i" class="h-16 w-full rounded-xl" /></div>
    <div v-else-if="visibleTasks.length" class="space-y-3">
      <article v-for="task in visibleTasks" :key="task.id" class="glass-card flex items-center gap-3 p-3">
