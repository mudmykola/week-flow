...
<template>
  <div class="min-h-screen bg-[var(--color-bg-alt)] text-[var(--color-text-primary)]">
    <aside
      class="fixed inset-y-0 left-0 z-40 w-64 border-r border-[var(--color-panel-border)] bg-[var(--color-panel-bg)] p-4 transition-transform lg:translate-x-0"
    ...
    >
      <div class="mb-7 flex items-center justify-between px-2 py-2">
        <NuxtLink to="/" class="flex items-center gap-3" @click="mobileOpen = false">
          ...
          <span class="font-display text-xl">WeekFlow</span>
        </NuxtLink>
      ...
      <button class="mb-4 flex w-full items-center gap-3 rounded-xl border border-[var(--color-panel-border)] px-3 py-2.5 text-left text-sm text-secondary hover:bg-black/[0.04] dark:hover:bg-white/[0.05]" @click="openCommand">
        ...
        <span class="flex-1">Пошук</span><kbd class="text-xs">⌘K</kbd>
      </button>
      ...
      <nav class="app-scrollbar flex max-h-[calc(100vh-13rem)] flex-col gap-1 overflow-y-auto">
        ...
        >
          <UIcon :name="item.icon" class="size-4.5" />{{ item.label }}
        </NuxtLink>
      </nav>

      <div class="absolute inset-x-4 bottom-4 flex items-center gap-3 rounded-xl border border-[var(--color-panel-border)] p-3">
        <NuxtImg v-if="user?.avatarUrl" :src="user.avatarUrl" width="32" height="32" class="size-8 rounded-full" alt="" />
        <div class="min-w-0 flex-1"><p class="truncate text-sm font-medium">{{ user?.name }}</p><p class="truncate text-xs text-secondary">{{ user?.email }}</p></div>
    ...
    <div class="lg:pl-64">
      <header class="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-[var(--color-panel-border)] bg-[color:var(--color-bg-alt)]/90 px-4 backdrop-blur-xl md:px-8">
        ...
        <NuxtLink to="/?new=1" class="inline-flex items-center gap-2 rounded-full bg-[var(--color-accent)] px-4 py-2 text-sm font-semibold text-white"><UIcon name="i-lucide-plus" class="size-4" /> Нова задача</NuxtLink>
      </header>
      <main><slot /></main>
    </div>
