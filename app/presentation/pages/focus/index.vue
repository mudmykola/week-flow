...
<template>
  <div class="focus-page app-container max-w-5xl">
    <PageHeader
      :title="$t('nav.focus')"
    ...
    <EmptyState
      ...
      ><UButton to="/?new=1">{{ $t('pages.focus.createTask') }}</UButton></EmptyState
    >
    <div
      v-else
      class="grid gap-3 lg:grid-cols-[minmax(0,1.35fr)_19rem]"
    >
      <section class="surface-card flex min-h-[21rem] flex-col items-center justify-center p-5 text-center">
        <span class="count-badge">{{ priorityLabels[selected.priority] }}</span>
        <h2 class="font-display mt-4 max-w-xl text-xl sm:text-2xl">{{ selected.title }}</h2>
        <p
          v-if="selected.note"
          class="text-secondary mt-2 max-w-xl text-sm"
        >
          {{ selected.note }}
        </p>
        <p class="font-display my-6 text-6xl tracking-tight tabular-nums sm:text-7xl">{{ display }}</p>
        <div class="flex flex-wrap justify-center gap-2">
          <UButton
            :icon="running ? 'i-lucide-pause' : 'i-lucide-play'"
            @click="toggleTimer"
            >{{ running ? $t('pages.focus.pause') : $t('pages.focus.start') }}</UButton
          ><UButton
            variant="soft"
            icon="i-lucide-rotate-ccw"
            @click="resetTimer"
            >{{ $t('pages.focus.reset') }}</UButton
          ><UButton
            color="success"
            variant="soft"
            icon="i-lucide-check"
      ...
      <aside class="surface-card p-2">
        <p class="text-secondary px-2 py-2 text-xs font-semibold tracking-wide uppercase">
          {{ $t('pages.focus.next') }}
        </p>
        <button
          v-for="task in tasks.slice(0, 8)"
          :key="task.id"
          class="flex w-full items-center gap-2 rounded-lg p-2.5 text-left text-sm"
          :class="
            task.id === selected?.id ? 'bg-[var(--color-bg-alt)] font-semibold' : 'hover:bg-[var(--color-bg-alt)]'
          "
