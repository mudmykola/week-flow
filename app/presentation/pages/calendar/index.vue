...
const activeView = computed(() => (route.query.view === 'timeline' ? 'timeline' : 'calendar'))
function selectView(next: 'calendar' | 'timeline') {
  void navigateTo({ path: '/calendar', query: next === 'calendar' ? {} : { view: next } })
}
const timelineRange = ref<14 | 30>(14)
const timelineStart = startOfDay(new Date())
const timelineDays = computed(() => Array.from({ length: timelineRange.value }, (_, i) => addDays(timelineStart, i)))
const timelineTasks = computed(() =>
  tasks.value
    .filter((task) => task.status !== 'done')
    .filter((task) => {
      const offset = differenceInCalendarDays(new Date(`${task.dueDate}T12:00:00`), timelineStart)
      return offset >= 0 && offset < timelineRange.value
    })
    .sort((a, b) => a.dueDate!.localeCompare(b.dueDate!))
)
function timelinePosition(task: Task) {
  return Math.max(0, differenceInCalendarDays(new Date(`${task.dueDate}T12:00:00`), timelineStart))
}
...
<template>
  <div class="calendar-page app-container">
    ...
    >
      <template #actions>
        <div
          ...
          role="group"
          :aria-label="$t('pages.calendar.mode')"
        >
          <button
            type="button"
            class="calendar-page__view-button"
            :class="{ 'calendar-page__view-button--active': activeView === 'calendar' }"
            @click="selectView('calendar')"
          >
            <UIcon name="i-lucide-calendar-days" />{{ $t('nav.calendar') }}
          </button>
          <button
            type="button"
            class="calendar-page__view-button"
            :class="{ 'calendar-page__view-button--active': activeView === 'timeline' }"
            @click="selectView('timeline')"
          >
            <UIcon name="i-lucide-gantt-chart" />{{ $t('nav.timeline') }}
          </button>
        </div>
        <div
    ...
    <template v-else>
      ...
      >
        <div class="flex rounded-xl border border-[var(--color-panel-border)] bg-[var(--color-panel-bg)] p-1">
          <button
            ...
            :class="
              timelineRange === option ? 'bg-[var(--color-text-primary)] text-[var(--color-bg)]' : 'text-secondary'
            "
            @click="timelineRange = option"
          >
      ...
      />
      <EmptyState
        v-else-if="!timelineTasks.length"
      ...
      >
        <div class="app-scrollbar overflow-x-auto">
          <div
            class="min-w-[900px]"
            :style="{ width: `${Math.max(900, timelineRange * 72 + 260)}px` }"
          >
            <div
              class="grid border-b border-[var(--color-panel-border)] bg-[var(--color-bg-alt)]"
              :style="{ gridTemplateColumns: `260px repeat(${timelineRange}, minmax(0, 1fr))` }"
            >
              ...
              >
                <p class="text-secondary text-[10px] uppercase">{{ format(day, 'EEE', { locale: dateLocale }) }}</p>
            ...
            >
              ...
              >
              <div
                v-for="(_day, index) in timelineDays"
                :key="index"
                class="relative border-l border-[var(--color-panel-border)]"
              >
                <div
                  v-if="index === timelinePosition(task)"
