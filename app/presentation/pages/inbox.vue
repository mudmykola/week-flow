...
<template><div class="app-container max-w-5xl"><PageHeader title="Inbox" description="Швидко зафіксуйте думку, розберете її пізніше." icon="i-lucide-inbox" :count="tasks.length" />
  <form class="surface-card mb-3 flex gap-2 p-2" @submit.prevent="capture"><input v-model="title" class="h-10 min-w-0 flex-1 bg-transparent px-2 outline-none" placeholder="Що потрібно не забути?" autofocus><UButton size="sm" type="submit" icon="i-lucide-plus" :loading="saving">Додати</UButton></form>
  <div v-if="loading" class="space-y-2"><USkeleton v-for="i in 4" :key="i" class="h-14 rounded-xl" /></div><EmptyState v-else-if="!tasks.length" title="Inbox розібрано" description="Усі думки перетворені на плани — чудова робота." icon="i-lucide-inbox-check"/><div v-else class="surface-card divide-y divide-[var(--color-panel-border)]"><article v-for="task in tasks" :key="task.id" class="flex items-center gap-3 p-3"><button title="Виконати" @click="complete(task)"><UIcon name="i-lucide-circle" class="size-5 text-secondary"/></button><NuxtLink :to="`/?week=${task.week}`" class="min-w-0 flex-1 truncate font-medium">{{ task.title }}</NuxtLink><span class="text-xs text-secondary">{{ task.week }}</span></article></div>
</div></template>
